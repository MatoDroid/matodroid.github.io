...
const DB_PATH = path.join(CURRENT_DIR, 'db.json');

function readDb() {
    if (!fs.existsSync(DB_PATH)) {
        const initialData = { menu: { tables: [], categories: [], menuItems: [] }, orders: {}, paidOrders: [] };
        fs.writeFileSync(DB_PATH, JSON.stringify(initialData, null, 2));
        return initialData;
    }
    const content = fs.readFileSync(DB_PATH, 'utf-8');
    return content ? JSON.parse(content) : { menu: { tables: [], categories: [], menuItems: [] }, orders: {}, paidOrders: [] };
}
...
app.use(express.static(path.join(CURRENT_DIR, '../'))); // Serve static files from root for specific requests if needed
app.use('/POS', express.static(path.join(CURRENT_DIR, '../'))); // Mount static files at /POS
...
// Chránený endpoint pre admin.html (iba pre adminov)
app.get("/POS/admin.html", authenticateToken, (req, res) => {
    if (req.user.role !== "admin") {
        return res.status(403).json({ message: "Nemáte oprávnenie na prístup k tejto stránke." });
    }
    res.sendFile(path.join(CURRENT_DIR, "../admin.html"));
});

// Chránený endpoint pre pos.html (iba pre používateľov s rolou 'user')
app.get("/POS/pos.html", authenticateToken, (req, res) => {
    if (req.user.role !== "user") {
        return res.status(403).json({ message: "Nemáte oprávnenie na prístup k tejto stránke." });
    }
    res.sendFile(path.join(CURRENT_DIR, "../pos.html"));
});
...
app.post('/menu', (req, res) => {
    try {
        ...
        res.json({ success: true, message: "Menu uložené" });
    } catch (error) {
...
app.post('/orders/paid', (req, res) => {
    try {
        const newPaidOrder = req.body; // Dáta od klienta

        const db = readDb();
        if (!db.paidOrders) db.paidOrders = [];
        db.paidOrders.push(newPaidOrder);
        writeDb(db);
